...
void main() {
    ...
    if(uCurrentMaterial.illum == 0) {
        ...
        for(int i = 0; i < light_point_arrsize; i++) {
            diff += Diffuse(nnormal, normalize(light_point[i].position - position), light_point[i].color);
        }
    ...
    //     for(int i = 0; i < light_point_arrsize; i++) {
    //         vec3 dir = normalize(light_point[i].position, -position);
    ...
    // }
        ...
        for(int i = 0; i < light_point_arrsize; i++) {
            vec3 dir = normalize(light_point[i].position - position);
            float dist = length(light_point[i].position - position);
